...
// THIVIN
// Same as the NSE - Type 4 Direct Solver , However, the redundant DOF's are removed
  ...
  if (flag == 0 || flag == 3)
  {
    ...
    for (i = 0; i < N_U; i++)
    {
      ...
      for (j = begin; j < end; j++)
      {
        ...
        pos++;
        // A12
        Entries[pos] = EntriesA12[j];
        KCol[pos] = KColA[j] + N_U;
        pos++;
        // A13
        Entries[pos] = EntriesA13[j];
        KCol[pos] = KColA[j] + 2 * N_U;
        pos++;
      }
    ...
    for (i = 0; i < N_U; i++)
    {
      ...
      {
        // A31
        Entries[pos] = EntriesA31[j];
        KCol[pos] = KColA[j];
        pos++;
        // A32
        Entries[pos] = EntriesA32[j];
        KCol[pos] = KColA[j] + N_U;
        pos++;
        // A33
        Entries[pos] = EntriesA33[j];
