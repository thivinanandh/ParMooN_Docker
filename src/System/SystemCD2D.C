...
// THIVIN -- INIT with custom Assembly Function
void TSystemCD2D::InitWithCustomAssembly(CoeffFct2D *BilinearCoeffs, BoundCondFunct2D *BoundCond, BoundValueFunct2D *BoundValue, int N_Terms, MultiIndex2D *Derivatives, int *SpacesNumbers, int CD_N_Matrices, int CD_N_Rhs,
										 ...
										 ManipulateFct2D *manipulate)
{
	...
	InitializeDiscreteFormsWithCustomFunction(DiscreteFormGalerkin,
											  BilinearCoeffs, N_Terms, Derivatives, SpacesNumbers, CD_N_Matrices, CD_N_Rhs,
											  CD_RowSpace, CD_ColumnSpace, CD_RhsSpace,
											  BilinearAssembleGalerkin, manipulate);

	// Assign it to the System Member
	DiscreteFormARhs = DiscreteFormGalerkin;
} // TSystemCD2D::Init
...
void TSystemCD2D::GetMassAndArea(TFEFunction2D *fefunction, double *parameters)
{
	...
	for (i = 0; i < N_Cells; i++)
	{
		...
		case TriaIsoparametric:
			...
			((TTriaIsoparametric *)F_K)->GetOrigFromRef(N_QFPoints, xi, eta, X, Y, AbsDetjk);
			break;
